/**
 * Pool Creator - Mining Pool Creation Module
 * Поддерживает FROST DKG и single-owner пулы
 * Интегрируется с существующей системой SatoshiFi
 */
...
class PoolCreator {
    ...
    // ДОБАВЛЕННЫЙ метод для проверки конфигурации
    ...
    async loadAvailableFrostSessions() {
        ...
        try {
            ...
            // ИСПРАВЛЕНИЕ: Проверяем и инициализируем DKG Manager
            if (!window.dkgManager) {
                console.log('DKG Manager not initialized, creating new instance...');

                if (typeof DKGManager !== 'undefined') {
                    window.dkgManager = new DKGManager();
                    // ВАЖНО: Ждем полной инициализации
                    await dkgManager.initialize();
                } else {
                    console.warn('DKGManager class not available');
                    return;
                }
            } else if (!dkgManager.initialized) {
                // Если существует, но не инициализирован
                console.log('DKG Manager exists but not initialized, initializing...');
                await dkgManager.initialize();
            }

            // ИСПРАВЛЕНИЕ: Всегда принудительно загружаем сессии
            console.log('Force loading user sessions for:', wallet.account);
            await dkgManager.loadUserSessions(wallet.account);

            // Проверяем результат загрузки
            const allSessions = Array.from(dkgManager.sessionsCache.values());
            console.log(`Found ${allSessions.length} total DKG sessions after loading`);

            if (allSessions.length === 0) {
                console.log('No DKG sessions found for user after loading');
                return;
            }

            // Получаем factory contract для проверки ролей
            const factory = contracts.getContract('factory');
            if (!factory) {
                console.warn('Factory contract not available for role checking');
                // Продолжаем без проверки ролей
                this.availableFrostSessions = allSessions.filter(session => {
                    const isFinalized = session.state === 2 || session.state === 4;
                    const isParticipant = session.participants && session.participants.includes(wallet.account);
                    return isFinalized && isParticipant;
                });
                return;
            }

            let POOL_MANAGER_ROLE;
            try {
                POOL_MANAGER_ROLE = await factory.POOL_MANAGER_ROLE();
            } catch (error) {
                console.warn('Cannot get POOL_MANAGER_ROLE from factory:', error);
                // Продолжаем без проверки ролей
                this.availableFrostSessions = allSessions.filter(session => {
                    const isFinalized = session.state === 2 || session.state === 4;
                    const isParticipant = session.participants && session.participants.includes(wallet.account);
            ...
            // Детальная диагностика каждой сессии
            for (const session of allSessions) {
                console.log(`Checking session ${session.id}:`, {
                    id: session.id,
                    state: session.state,
                    stateText: this.getStateText(session.state),
                            participants: session.participants?.length || 0,
                            participantsList: session.participants,
                            isCurrentUserParticipant: session.participants ? session.participants.includes(wallet.account) : false,
                            threshold: session.threshold,
                            groupPubkey: session.groupPubkey ? 'Present' : 'Missing'
                });

                // Проверяем роли всех участников
                if (session.participants && session.participants.length > 0) {
                    console.log(`Checking POOL_MANAGER roles for session ${session.id}...`);

                    const roleChecks = [];
                    for (const participant of session.participants) {
                        try {
                            const hasRole = await factory.hasRole(POOL_MANAGER_ROLE, participant);
                            roleChecks.push({ address: participant, hasPoolManagerRole: hasRole });
                            console.log(`${participant}: POOL_MANAGER = ${hasRole}`);
                        } catch (error) {
                            console.warn(`Error checking role for ${participant}:`, error);
                            roleChecks.push({ address: participant, hasPoolManagerRole: false, error: error.message });
                        }
                    }

                    session.participantRoleChecks = roleChecks;
                    session.allParticipantsAuthorized = roleChecks.every(check => check.hasPoolManagerRole);

                    console.log(`Session ${session.id} role summary:`, {
                        totalParticipants: session.participants.length,
                        authorizedParticipants: roleChecks.filter(check => check.hasPoolManagerRole).length,
                                allAuthorized: session.allParticipantsAuthorized
                    });
                }
            }

            // Фильтрация с проверкой авторизации участников
            this.availableFrostSessions = allSessions.filter(session => {
                const isFinalized = session.state === 2 || session.state === 4;
                const isParticipant = session.participants && session.participants.includes(wallet.account);
                const hasParticipants = session.participants && session.participants.length > 0;
                const hasThreshold = session.threshold > 0;
                const allAuthorized = session.allParticipantsAuthorized === true;

                console.log(`Session ${session.id} final filter check:`, {
                    isFinalized,
                    isParticipant,
                    hasParticipants,
                    hasThreshold,
                    allAuthorized,
                    willInclude: isFinalized && isParticipant && hasParticipants && hasThreshold && allAuthorized
                });

                return isFinalized && isParticipant && hasParticipants && hasThreshold && allAuthorized;
            });

            console.log(`Loaded ${this.availableFrostSessions.length} available FROST sessions for pool creation`);

            this.availableFrostSessions.forEach((session, index) => {
                console.log(`Available Session ${index + 1}:`, {
                    id: session.id,
                    participants: session.participants.length,
                    threshold: session.threshold,
                    state: this.getStateText(session.state),
                            authorizedParticipants: session.participantRoleChecks?.filter(check => check.hasPoolManagerRole).length || 0
                });
            });
    ...
    // ДОБАВЛЕННЫЙ метод для принудительного обновления FROST сессий
    async refreshFrostSessions() {
        try {
            ...
            // Принудительно перезагружаем сессии
            if (window.dkgManager) {
                dkgManager.sessionsCache.clear();
                await dkgManager.loadUserSessions(wallet.account);
            }

            // Перезагружаем доступные сессии
            await this.loadAvailableFrostSessions();
    ...
    // ИСПРАВЛЕННЫЙ метод расчета газа
    async calculateGasEstimate() {
        ...
        try {
            ...
            if (this.poolData.governanceType === 'frost') {
                // ИСПРАВЛЕНО: используем extractPubkeyFromFrostSession для получения pubX/pubY
            ...
            const estimatedCost = (gasEstimate * parseInt(gasPrice, 16)) / 1e18;

            if (gasElement) gasElement.textContent = gasEstimate.toString();
            if (costElement) costElement.textContent = `~${estimatedCost.toFixed(6)} ETH`;
            ...
            setTimeout(() => {
                if (gasElement) gasElement.textContent = '~2,500,000';
                if (costElement) costElement.textContent = '~0.05 ETH';
            }, 1000);
        }
    }

    // ПРАВИЛЬНЫЙ метод декомпрессии с использованием встроенных возможностей
    extractPubkeyFromFrostSession(session) {
        ...
        try {
            ...
            // Если ключ сжатый (33 байта = 66 hex символов)
            if (pubkeyHex.length === 66) {
                console.log('Compressed public key detected');

                // Извлекаем X координату и флаг четности Y
                const prefix = parseInt(pubkeyHex.slice(0, 2), 16);
                const xHex = pubkeyHex.slice(2);
                const pubX = '0x' + xHex;

                // Вычисляем Y из X используя уравнение кривой secp256k1: y^2 = x^3 + 7
                const x = BigInt(pubX);
                ...
                // Вычисляем квадратный корень mod p (используем формулу Tonelli-Shanks упрощенно)
                // Для secp256k1 можно использовать: y = y_squared^((p+1)/4) mod p
                const exp = (p + BigInt(1)) / BigInt(4);
                let y = this.modPow(y_squared, exp, p);

                // Проверяем четность и корректируем если нужно
            ...
            // Если ключ несжатый (65 байт = 130 hex символов, первый байт 04)
            ...
            // Если ключ в формате X||Y (64 байта = 128 hex символов)
    ...
    async generatePayoutScript() {
        try {
            ...
            // ИСПРАВЛЕНО: Кодирование через codec
    ...
    async generatePayoutScriptFromFrost() {
        try {
            ...
            // Извлекаем публичный ключ из FROST сессии
            const { pubX, pubY } = this.extractPubkeyFromFrostSession(this.selectedFrostSession);
            ...
            // Конвертируем в формат для Bitcoin библиотеки (uncompressed)
            const pubkeyHex = '04' +
            pubX.toString(16).padStart(64, '0') +
            pubY.toString(16).padStart(64, '0');

            const pubkeyBuffer = Buffer.from(pubkeyHex, 'hex');
            const btcNetwork = bitcoinAddressCodec.getNetwork();

            // ИСПРАВЛЕНО: Создаём P2WPKH (bech32) вместо P2PKH
    ...
    async createFrostPool() {
        if (!this.selectedFrostSession) {
            throw new Error('FROST сессия не выбрана');
        }
        ...
        const result = {
            ...
            poolId: this.poolData.poolId,  // ✅ ДОБАВЛЕНО для mining simulator
    ...
    async createSingleOwnerPool() {
        ...
        const result = {
            ...
            txHash: receipt.transactionHash,
            addresses: {
                core: poolCreatedEvent.args.poolCore,
                mpToken: poolCreatedEvent.args.mpToken
            },
...
// ДОБАВЛЕННЫЕ глобальные методы для безопасности
window.PoolCreatorAPI = {
    // Безопасный метод для проверки доступности API
    ...
    // Метод для получения полного URL с валидацией
    getSafeUrl(endpoint) {
        if (!CONFIG.API.ENDPOINTS[endpoint]) {
    ...
    // Метод для логирования всех API вызовов
